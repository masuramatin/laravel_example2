...
<div id='con'>
    <div align="center" ><a href="{{action('BlogsController@create')}}"><h4>Add Item</h4></a></div>
    <table width="90%" border="1" class="table table-striped">
        ...
        <tr>
           <td>{{$blog['subject']}}</td>
            ...
            <td><img src="{{ url('/') }}/img/{{$blog['name']}}" height="100px" width="100px" /></td>
            <td>
            <table border=1>
            <tr> <td>
            @if(Auth::user()->type==3 || Auth::user()->type==2)
            <a href="{{action('BlogsController@edit', $blog['id'])}}"  class="btn btn-warning">Edit</a>   </td>
            @endif
            @if(Auth::user()->type==3)
           <td>
   <form action="{{ route('blog.destroy', $blog['id']) }}" method="get">
            ...
            @endif
            <td>
              <a href="{{action('BlogsController@show', $blog['id'])}}"  class="btn btn-info">View</a>
             </td>
             </tr>
             </table>
             </td>
